...
class PostDetailAndReplyCreate(LoginRequiredMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        subject = 'Hi! You have a new answer!'
        message = f'{self.request.user.username} sent you a response!'
        html_attachement = f'<a href="{link}">View here!</a>'
...
class ReplyApproveView(UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        link = reverse_lazy('post', kwargs={'pk': self.kwargs['pk']})
        subject = 'Hi! The author approved your response!'
        message = f'{self.request.user.username} approved your response!'
        html_attachement = f'<a href="{link}">View!</a>'
        recipient = [Reply.objects.filter(pk=self.kwargs['reply_pk']).select_related('author').only('author__email')[0].author.email]
        print(recipient)
        send_mail(
            subject=subject,
            message=message,
